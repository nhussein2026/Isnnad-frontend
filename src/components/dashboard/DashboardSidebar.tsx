...
export default function DashboardSidebar() {
  ...
  const links = [
    { to: "/dashboard", label: "لوحة التحكم", icon: <LayoutDashboard className="h-5 w-5" /> },
    { to: "/dashboard/courses", label: "المواد", icon: <BookOpen className="h-5 w-5" /> },
    { to: "/dashboard/new-task", label: "إنشاء مهمة جديدة", icon: <PlusCircle className="h-5 w-5" /> },
    { to: "/dashboard/cart", label: "سلة المهام", icon: <ShoppingCart className="h-5 w-5" /> },
    { to: "/dashboard/settings", label: "الإعدادات", icon: <Settings className="h-5 w-5" /> },
  ];
          ...
          {links.map(link => (
            <Link
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-gray-100 ${
                location.pathname === link.to ? "bg-gray-100 text-primary" : "text-gray-700"
              }`}
            >
              {link.icon}
              {link.label}
